...
def model_testing(X_train,X_test,y_train,y_test,max_components_Lv,model=None):
    ...
    if model is not None:
        ...
        y_pred_array = testing_timeseries_(X_test_std, pls, x_col, simple=False, number_lags=9)
...
def execute_modelling():
    ...
    for max_components_Lv in range(1,min(sum(lag_rule),max_components_Lv_)):
        ...
        varaince_explined_x.append([max_components_Lv,pctvar_X])
        Y_var = np.var(pls_2.y_scores_, axis=0)
        Y_total_var = np.var(Y_train_2, axis=0).sum()
        pctvar_Y = Y_var / Y_total_var * 100
        varaince_explined_y.append([max_components_Lv,pctvar_Y])
...
def execute_testing():
    ...
    model_testing(X_train,df4_lagged_x,Y_train,df4_lagged_y,max_components_Lv)
    model_testing(X_train, df1_lagged_x, Y_train, df1_lagged_y, max_components_Lv)
